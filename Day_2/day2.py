...
def part1(data):
    id_count = 0
    sum_of_powers = 0
    blueRegex = re.compile(r'(\d+) blue')
    redRegex = re.compile(r'(\d+) red')
    greenRegex = re.compile(r'(\d+) green')
    data = parseData(data)
    for index,game in enumerate(data):
        ...
        green_count = 0
        for set in game[1]:
            blue = blueRegex.findall(set)
            red = redRegex.findall(set)
            green = greenRegex.findall(set)
            if len(blue) > 0 and int(blue[0]) <= requirements['blue']:
                blue_count = max(blue_count,int(blue[0]))
                valid += 1
            elif len(blue) == 0:
                valid += 1
            if len(red) > 0 and int(red[0]) <= requirements['red']:
                red_count = max(int(red[0]),red_count)
                valid += 1
            elif len(red) == 0:
                valid += 1
            if len(green) > 0 and int(green[0]) <= requirements['green']:
                green_count = max(int(green[0]),green_count)
                valid += 1
            elif len(green) == 0:
                valid += 1
        if valid == 3 * len(game[1]):
            id_count += (index + 1)
        sum_of_powers += (blue_count * red_count * green_count)
    return id_count, sum_of_powers
print(part1(data))